...
test("optimize updates generated context files without backups", () => {
  const root = tempRepo();
  fs.mkdirSync(path.join(root, "backend", "app"), { recursive: true });
  fs.mkdirSync(path.join(root, "frontend", "src", "app"), { recursive: true });
  fs.writeFileSync(path.join(root, "backend", "requirements.txt"), "fastapi\nsqlalchemy\n", "utf8");
  ...
  fs.writeFileSync(path.join(root, "backend", "app", "routes.py"), "from .main import FastAPI\n", "utf8");
  ...
  assert.equal(second.status, 0, second.stderr);
  assert.ok(fs.readFileSync(path.join(root, ".prismo", "backend-summary.md"), "utf8").includes("backend/app/routes.py"));
