...
async function runCli(argv) {
  ...
  if (!["dev", "setup", "scan", "optimize", "context", "cc", "usage", "watch", "demo"].includes(command)) {
    throw new Error(`Unknown command: ${command}. Try: prismo dev, prismo setup, prismo scan, prismo optimize, prismo context, prismo cc, or prismo usage`);
  }
  ...
  if (command === "cc") {
    ...
    const subcommand = positional[0] && ["list", "last", "all"].includes(positional[0].toLowerCase()) ? positional[0].toLowerCase() : "latest";
