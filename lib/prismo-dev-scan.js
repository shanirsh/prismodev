...
async function runCli(argv) {
  ...
  if (!["dev", "init", "doctor", "firewall", "benchmark", "shield", "mcp", "connect", "sync", "status", "disconnect", "setup", "scan", "optimize", "context", "cc", "cursor", "receipt", "instructions", "timeline", "replay", "boundaries", "usage", "guard", "watch", "demo"].includes(command)) {
    throw new Error(`Unknown command: ${command}. Try: prismo connect, prismo guard, prismo sync, prismo doctor, prismo watch, prismo receipt, prismo benchmark, prismo shield, prismo mcp, prismo firewall, prismo init, prismo scan, prismo optimize, prismo context, prismo cc, prismo cursor, or prismo usage`);
  }
