...
Usage:
  ...
  prismo watch [codex|claude|all] [--json] [--once] [--report] [--rescue] [--guardrails] [--throttle] [--budget N] [--redact-paths] [--interval N] [path]
  prismo demo
...
async function runCli(argv) {
  ...
  if (command === "usage" || command === "watch") {
    ...
    const tokenBudget = parseTokenBudget(budgetIndex >= 0 ? rest[budgetIndex + 1] : null);
    const usageOptions = {
      tool,
      cwd: path.resolve(target),
      limit,
      ...
      throttle: rest.includes("--throttle"),
