module.exports = function createCloudSync(deps) {
  ...
  function sanitizeSession(session, repo) {
    ...
    const accounting = {
      countsTowardTotals: session.accounting?.countsTowardTotals !== false,
      sourceOfTruth: session.accounting?.sourceOfTruth || (session.confidence === "exact-local-log" ? "local-log" : "local-estimate"),
      canonicalUsageId: session.accounting?.canonicalUsageId || session.sessionId || null,
      totalsIncludeChildRuns: typeof session.accounting?.totalsIncludeChildRuns === "boolean" ? session.accounting.totalsIncludeChildRuns : null,
    };
    return {
      ...
      run: {
        role: session.run?.role || "leader",
        parentSessionId: session.run?.parentSessionId || null,
        workerId: session.run?.workerId || null,
        toolUseId: session.run?.toolUseId || null,
        permissionRequestId: session.run?.permissionRequestId || null,
      },
      accounting,
    };
  }
  ...
  function buildSyncPayload(rootDir = process.cwd(), options = {}) {
    ...
    const aggregate = sessions.reduce((acc, session) => {
      acc.sessions += 1;
      if (session.accounting?.countsTowardTotals === false) return acc;
