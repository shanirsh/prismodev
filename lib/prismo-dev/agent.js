module.exports = function createAgent(deps) {
  ...
  // Self-initiated planner repairs are reported through the auto-detect
  // channel: the device token cannot create workspace action rows.
  ...
  async function runAgentOnce(rootDir = process.cwd(), options = {}) {
    ...
    if (options.planRepairs && repairPlanner) {
      try {
        ...
        await reportPlanner(config, plannerResult, mode, options);
      } catch (error) {
