module.exports = function createInstructionsAudit(deps) {
  ...
  function backupPathFor(filePath) {
    const base = `${filePath}.prismo-backup`;
    if (!fs.existsSync(base)) return base;
    return `${base}.${new Date().toISOString().replace(/[:.]/g, "-")}`;
  }

  function buildInstructionsApplyReport(result) {
    const lines = [];
    lines.push("# Prismo Instructions Apply Report");
    lines.push("");
    lines.push(`Generated: ${result.generatedAt}`);
    lines.push(`Mode: ${result.dryRun ? "dry run" : "applied"}`);
    lines.push(`Scanned path: ${result.scannedPath}`);
    lines.push("");
    lines.push("## Safe Changes");
    if (result.safeChanges.length) {
      result.safeChanges.forEach((change) => {
        lines.push(`- ${change.file}: remove duplicate line ${change.line}: ${change.text}`);
      });
    } else {
      lines.push("- No exact duplicate instruction lines found.");
    }
    lines.push("");
    lines.push("## Backups");
    if (result.backups.length) {
      result.backups.forEach((backup) => lines.push(`- ${backup.file} -> ${backup.backup}`));
    } else {
      lines.push("- No backups written.");
    }
    lines.push("");
    lines.push("## Recommendations Only");
    const recommendations = [
      ...result.recommendations.observablyViolated,
      ...result.recommendations.partialCompliance,
      ...result.recommendations.influenceUnknown,
      ...result.recommendations.trimCandidates,
    ];
    if (recommendations.length) {
      ...
      lines.push("- No recommendation-only rules found.");
    }
    lines.push("");
    lines.push("## Next");
  ...
  function buildInstructionsApply(options = {}) {
    const root = options.cwd || process.cwd();
    const dryRun = options.dryRun !== false;
    const audit = buildInstructionsAudit(options);
    const fileOrder = new Map(audit.files.map((file, index) => [file.path, index]));
    const groups = new Map();
    for (const rule of audit.rules) {
      if (!rule.normalized) continue;
      if (!groups.has(rule.normalized)) groups.set(rule.normalized, []);
      groups.get(rule.normalized).push(rule);
    }
    ...
    for (const rules of groups.values()) {
      ...
      const sorted = rules.slice().sort((a, b) => {
        const orderA = fileOrder.has(a.file) ? fileOrder.get(a.file) : 999;
        const orderB = fileOrder.has(b.file) ? fileOrder.get(b.file) : 999;
        return orderA - orderB || a.line - b.line;
      });
      sorted.slice(1).forEach((rule) => removals.push(rule));
    }

    const safeChanges = removals.map((rule) => ({
      file: rule.file,
      line: rule.line,
      text: rule.text,
      reason: "exact duplicate instruction line",
    }));
    const backups = [];
    const writtenFiles = [];
    ...
    if (!dryRun && removals.length) {
      ...
      for (const rule of removals) {
        if (!removalsByFile.has(rule.file)) removalsByFile.set(rule.file, new Set());
        removalsByFile.get(rule.file).add(rule.line);
      }
      for (const [rel, linesToRemove] of removalsByFile.entries()) {
        const file = filesByPath.get(rel);
        if (!file) continue;
        const fullPath = path.join(root, rel);
        const backup = backupPathFor(fullPath);
        fs.copyFileSync(fullPath, backup);
        const originalLines = (readIfText(fullPath) || "").split(/\r?\n/);
        const filtered = originalLines.filter((_, index) => !linesToRemove.has(index + 1));
        fs.writeFileSync(fullPath, filtered.join("\n"), "utf8");
        backups.push({ file: rel, backup: path.relative(root, backup) });
        writtenFiles.push(rel);
      }
    }

    const recommendations = {
      observablyViolated: audit.rules.filter((rule) => rule.status === "observably-violated"),
      partialCompliance: audit.rules.filter((rule) => rule.status === "partial-compliance"),
      influenceUnknown: audit.rules.filter((rule) => rule.status === "influence-unknown"),
      trimCandidates: audit.rules.filter((rule) => rule.status === "trim-candidate"),
    };
    ...
    const result = {
      ...
      auditSummary: audit.summary,
      safeChanges,
      changedFiles: dryRun ? [] : writtenFiles,
      backups,
      recommendations,
      reportPath: dryRun ? null : ".prismo/instructions-apply-report.md",
      next: [
        dryRun && safeChanges.length ? "Run without --dry-run to apply exact duplicate cleanup." : null,
        !dryRun && safeChanges.length ? "Review the git diff before continuing." : null,
        recommendations.observablyViolated.length ? "Move observably violated rules into live guardrails/firewall rules instead of deleting the intent." : null,
        recommendations.partialCompliance.length ? "Rewrite partial-compliance rules as observable acceptance checks." : null,
        recommendations.influenceUnknown.length ? "Leave influence-unknown rules alone unless you run an ablation window." : null,
        !safeChanges.length ? "No safe automatic cleanup found; use instructions audit/ablate for recommendations." : null,
      ].filter(Boolean),
    };

    if (!dryRun) {
      const prismoDir = path.join(root, ".prismo");
      fs.mkdirSync(prismoDir, { recursive: true });
      fs.writeFileSync(path.join(prismoDir, "instructions-apply-report.md"), buildInstructionsApplyReport(result), "utf8");
    }

    return result;
  }

  function renderInstructionsApplyTerminal(result) {
    const lines = [];
    lines.push("");
    lines.push("Prismo Instructions Apply");
    lines.push("");
    lines.push(`Mode: ${result.dryRun ? "dry run (no files changed)" : "applied"}`);
    lines.push(`Safe duplicate removals: ${result.safeChanges.length}`);
    lines.push("");
    lines.push("Safe Changes");
    if (result.safeChanges.length) {
      result.safeChanges.slice(0, 12).forEach((change) => {
        lines.push(`- ${change.file}:${change.line} ${change.text}`);
      });
    } else {
      lines.push("- No exact duplicate instruction lines found.");
    }
    ...
    lines.push("Recommendations Only");
    lines.push(`- Observably violated: ${result.recommendations.observablyViolated.length}`);
    lines.push(`- Partial compliance: ${result.recommendations.partialCompliance.length}`);
    lines.push(`- Influence unknown: ${result.recommendations.influenceUnknown.length}`);
    lines.push(`- Trim candidates: ${result.recommendations.trimCandidates.length}`);
