module.exports = function createReceipt(deps) {
  ...
  function buildContextEfficiency(session, repeatedReads, artifactGroups, repeatedCommands, rootCause) {
    const tokenBase = Math.max(Number(session.displayTokens || session.contextTokens || session.estimatedTotalTokens || 0), 1);
    const focusedFileSignals = Math.min(5, (repeatedReads || []).length);
    const failureSignals = Math.min(4, Number(session.failureMentions || 0));
    const toolUseSignals = Math.min(4, Number(session.toolCalls || 0));
    const rootCauseSignal = rootCause?.cause && rootCause.cause !== "healthy-or-low-signal" ? 1 : 0;
    const grossDecisionSignals = focusedFileSignals + failureSignals + toolUseSignals + rootCauseSignal;

    const repeatedReadDrag = Math.min(8, (repeatedReads || []).reduce((sum, item) => sum + Math.max(0, Number(item.count || 0) - 1), 0) / 4);
    const artifactDrag = Math.min(6, (artifactGroups || []).reduce((sum, item) => sum + Number(item.count || 0), 0) / 6);
    const outputDrag = Math.min(8, Number(session.estimatedToolTokens || 0) / 25000);
    const commandDrag = Math.min(5, (repeatedCommands || []).reduce((sum, item) => sum + Math.max(0, Number(item.count || 0) - 1), 0) / 2);
    const loopDrag = session.loopSuspicion ? 4 : 0;
    const dragSignals = repeatedReadDrag + artifactDrag + outputDrag + commandDrag + loopDrag;

    const netDecisionSignals = Math.max(0, grossDecisionSignals - (dragSignals * 0.35));
    const decisionDensityPer1k = netDecisionSignals / (tokenBase / 1000);
    const rating = decisionDensityPer1k >= 0.08
      ? "high"
      : decisionDensityPer1k >= 0.03
        ? "medium"
        : "low";
    const dragFactors = [];
    if (repeatedReadDrag >= 1) dragFactors.push("repeated-reads");
    if (artifactDrag >= 1) dragFactors.push("artifact-leaks");
    if (outputDrag >= 1) dragFactors.push("tool-output-flood");
    if (commandDrag >= 1 || loopDrag) dragFactors.push("command-loop");

    return {
      rating,
      metric: "heuristic decision signals per 1k displayed/context tokens",
      tokenBase,
      grossDecisionSignals: Number(grossDecisionSignals.toFixed(2)),
      dragSignals: Number(dragSignals.toFixed(2)),
      netDecisionSignals: Number(netDecisionSignals.toFixed(2)),
      decisionDensityPer1k: Number(decisionDensityPer1k.toFixed(4)),
      signals: {
        focusedFiles: focusedFileSignals,
        failuresIdentified: failureSignals,
        toolUses: toolUseSignals,
        rootCause: rootCauseSignal,
      },
      drag: {
        repeatedReads: Number(repeatedReadDrag.toFixed(2)),
        artifacts: Number(artifactDrag.toFixed(2)),
        toolOutput: Number(outputDrag.toFixed(2)),
        repeatedCommands: Number(commandDrag.toFixed(2)),
        loops: loopDrag,
      },
      dragFactors,
      note: "This is an approximate output-density signal, not a ground-truth measure of reasoning quality.",
    };
  }
  ...
  function buildReceiptSession(session) {
    ...
    const rootCause = chooseRootCause(session);
    const contextEfficiency = buildContextEfficiency(session, repeatedReads, artifactGroups, repeatedCommands, rootCause);
  ...
  function buildReceipt(options = {}) {
    ...
    const aggregate = sessions.reduce((acc, session) => {
      ...
      acc.grossDecisionSignals += session.contextEfficiency?.grossDecisionSignals || 0;
      acc.dragSignals += session.contextEfficiency?.dragSignals || 0;
      acc.netDecisionSignals += session.contextEfficiency?.netDecisionSignals || 0;
      return acc;
    }, {
      sessions: 0,
      displayTokens: 0,
      ...
      repeatedCommands: 0,
      artifactMentions: 0,
      grossDecisionSignals: 0,
      dragSignals: 0,
      netDecisionSignals: 0,
    });
    aggregate.decisionDensityPer1k = Number((aggregate.netDecisionSignals / Math.max(aggregate.displayTokens / 1000, 1)).toFixed(4));
    aggregate.contextEfficiency = aggregate.decisionDensityPer1k >= 0.08
      ? "high"
      : aggregate.decisionDensityPer1k >= 0.03
        ? "medium"
        : "low";
  ...
  function renderReceiptTerminal(receipt) {
    ...
    lines.push(`- ${primary.contextEfficiency.rating} (${primary.contextEfficiency.decisionDensityPer1k} decision signals / 1k tokens)`);
    lines.push(`- Signals: ${primary.contextEfficiency.netDecisionSignals} net / ${primary.contextEfficiency.grossDecisionSignals} gross; drag ${primary.contextEfficiency.dragSignals}`);
    if (primary.contextEfficiency.dragFactors.length) {
      lines.push(`- Main drag: ${primary.contextEfficiency.dragFactors.join(", ")}`);
    } else {
      lines.push("- Main drag: none detected");
    }
    lines.push("");
