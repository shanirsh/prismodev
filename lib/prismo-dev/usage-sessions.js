...
function hasNestedFlag(value, keys, depth = 0) {
  if (!value || typeof value !== "object" || depth > 5) return false;
  for (const key of keys) {
    if (value[key] === true) return true;
    if (typeof value[key] === "string" && /^(true|yes|worker|swarm_worker)$/i.test(value[key])) return true;
  }
  return Object.values(value).some((child) => hasNestedFlag(child, keys, depth + 1));
}

function updateRunMetadata(session, row) {
  const parentSessionId = findNestedString(row, [
    "parentSessionId",
    "parent_session_id",
    "parentSessionID",
    "parent_session",
  ...
  const workerId = findNestedString(row, ["workerId", "worker_id", "swarmWorkerId", "swarm_worker_id"]);
  const toolUseId = firstString(
    findNestedString(row, ["toolUseId", "tool_use_id", "toolUseID"]),
    row.toolUseID,
  );
  const permissionRequestId = findNestedString(row, [
    "permissionRequestId",
    "permission_request_id",
    "permissionId",
    "permission_id",
  ]);
  const explicitRole = findNestedString(row, ["runRole", "run_role", "role", "agentRole", "agent_role"]);
  const isWorker = Boolean(
    parentSessionId ||
    workerId ||
    /^(worker|swarm_worker|subagent|child)$/i.test(explicitRole || "") ||
    hasNestedFlag(row, ["isSwarmWorker", "swarmWorker", "is_worker", "isWorker"]),
  );

  if (isWorker) session.run.role = "worker";
  if (parentSessionId && parentSessionId !== session.sessionId) session.run.parentSessionId = parentSessionId;
  if (workerId) session.run.workerId = workerId;
  if (toolUseId) session.run.toolUseId = toolUseId;
  if (permissionRequestId) session.run.permissionRequestId = permissionRequestId;
}
...
function analyzeSessionFile(filePath, tool) {
  ...
  const session = {
    ...
    accounting: {
      ...
      canonicalUsageId: null,
      totalsIncludeChildRuns: null,
    },
  };
  ...
  for (const row of rows) {
    if (tool === "claude-code") updateRunMetadata(session, row);
  ...
  if (tool === "claude-code") {
    const isWorker = session.run.role === "worker" && Boolean(session.run.parentSessionId);
    session.accounting = {
      countsTowardTotals: !isWorker,
      sourceOfTruth: isWorker ? "parent-session-rollup" : "claude-code-jsonl",
      canonicalUsageId: isWorker ? session.run.parentSessionId : session.sessionId,
      totalsIncludeChildRuns: isWorker ? null : null,
    };
  } else if (tool === "codex") {
    session.accounting = {
      countsTowardTotals: true,
      sourceOfTruth: session.exactAvailable ? "codex-token-count" : "local-estimate",
      canonicalUsageId: session.sessionId,
      totalsIncludeChildRuns: null,
    };
  }
